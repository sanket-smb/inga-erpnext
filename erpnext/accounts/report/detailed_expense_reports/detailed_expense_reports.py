...
def execute(filters=None):
	...
	#stock_in_hands_end
	month_wise_stock_in_hands = get_month_wise_stock_in_hands(filters)
        # test_data = {
        #         "account_name": "'" + _("Test") + "'",
        #         "account": "'" + _("Test") + "'",
	...
	#data.append(["Stock In Hand - IBS","Stock In Hand - IBS-"+str(month_wise_stock_in_hands),month_wise_stock_in_hands])
...
#stock_in_hands_start
def get_month_wise_stock_in_hands(filters):
	lr= frappe.db.sql("""select lft,rgt from `tabAccount` where name='Stock In Hand - IBS' """, as_dict=1);
	lft = lr[0]["lft"]
	rgt = lr[0]["rgt"]
	
	month_wise_stock_in_hands = frappe.db.sql("""select posting_date, account,sum(debit) as debit from `tabGL Entry` where MONTHNAME(posting_date)=%s and YEAR(posting_date)=%s and company="Inga Beauty salon" and account in (select name from tabAccount where lft>=%s and rgt<=%s and docstatus<2)""",(filters.month_mp,filters.from_fiscal_year,lft,rgt), as_dict=1) 

	return month_wise_stock_in_hands[0]["debit"]  if month_wise_stock_in_hands[0]["debit"] else 0 
#stock_in_hand_end.
#suresh_custom_end
