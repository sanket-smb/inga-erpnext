...
def get_data(company, root_type, balance_must_be, period_list, filters=None,
        ...
        if out:
                # test_data = {
                #         "account": "Total Expense (Debit)1",
                #         "currency": "KWD",
                #         "jan_2019": 15579.25,
                #         "total": 52512.118,
                #         "account_name": "Total Expense (Debit)1",
                #         "has_value": True,
                #         "opening_balance":0,
                #         "parent_account":"Direct Expenses - IBS",
                #         "indent": 2.0
                # }
                update_data = []
                acc_list = ["Stock In Hand - IBS","Stock Received But Not Billed - IBS"]
                for acc in acc_list:
                        #print(filters)
                        d = get_other_stock_account(period_list,filters,acc,company_currency)
                        update_data.append(d)
                        out.append(d)
                #print(update_data)
                add_total_row(out, root_type, balance_must_be, period_list, company_currency,update_data)

        return out

def get_other_stock_account(period_list,filters=None,account=None,company_currency=None):
        year_start_date = period_list[0]["year_start_date"].strftime("%Y-%m-%d")
        year_end_date = period_list[-1]["year_end_date"].strftime("%Y-%m-%d")
        month_total = frappe.db.sql("""select sum(debit) from `tabGL Entry` where account=%s and monthname(posting_date)=%s and year(posting_date)=%s""",(account,filters.get("month_mp"),filters.get("from_fiscal_year")))
        year_total = frappe.db.sql("""select sum(debit) from `tabGL Entry` where account=%s and year(posting_date)=%s""",(account,filters.get("from_fiscal_year")))
        print(month_total)
        mon_total = y_total = 0
        if len(month_total) >= 1:
                mon_total = month_total[0][0] or 0
        if len(year_total) >= 1:
                y_total = year_total[0][0]
        #print(''+str(filters[month_map]))
        month_name_year = get_month_key(filters.get("month_mp"),filters.get("from_fiscal_year"))
        stock_data = dict(
                account = account,
                currency = company_currency,
                total = y_total,
                account_name = get_account_label_name(account),
                opening_balance = 0,
                parent_account = "Direct Expenses - IBS",
                indent = 2.0
        )
        stock_data[month_name_year] = mon_total
        return stock_data

def get_account_label_name(acc):
        if acc == "Stock In Hand - IBS":
                return "Purchase Paid"
        else:
                return "Purchase Unpaid"
...
def get_month_key(month,year):
        ...
        print(month_dict)
        return month_dict.get(month)+str(year)
...
def calculate_values(accounts_by_name, gl_entries_by_account, period_list, accumulated_values, ignore_accumulated_values_for_fy):
        for entries in gl_entries_by_account.values():
                for entry in entries:
                        if not entry.account in ["Expenses Included In Valuation - IBS","Cost of Goods Sold - IBS","Stock Adjustment - IBS"]:
...
def add_total_row(out, root_type, balance_must_be, period_list, company_currency,stock_data):
        total_row = {
        ...
        for row in out:
                if not row.get("parent_account"):
                        for period in period_list:
                                ...
                                for new_row in stock_data:
                                        total_row[period.key] += new_row.get(period.key,0.0)
                                row[period.key] = 0.0

                        total_row.setdefault("total", 0.0)
                        total_row["total"] += flt(row["total"])
                        for new_row in stock_data:
                                total_row["total"] += flt(new_row["total"])
                        row["total"] = ""
...
def get_accounts(company, root_type):
        return frappe.db.sql("""select name, account_number, parent_account, lft, rgt, root_type, report_type, account_name from `tabAccount`
                where company=%s and root_type=%s and name not in ("Cost of Goods Sold - IBS","Expenses Included In Valuation - IBS","Stock Adjustment - IBS") order by lft""", (company, root_type), as_dict=True)
...
def get_columns(month_mp,periodicity, period_list, accumulated_values=1, company=None):
        columns = [{
                "fieldname": "account",
                "label": _("Account"),
                "fieldtype": "Link",
        ...
        for period in period_list:
		period_label_var = period.label
		if(period_label_var[:3] == month_mp[:3] ):
		        columns.append({
